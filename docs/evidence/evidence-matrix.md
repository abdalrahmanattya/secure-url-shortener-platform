...
This matrix is the source of truth for what a reviewer may safely infer from
...
| `302` resolution, `410` lifecycle, tombstones, and code non-reuse | `src/secure_shortener/main.py`, `src/secure_shortener/service.py`, `tests/test_api.py` | Locally verified | Verify recovery and retention policy |
...
| Local incident/rollback procedure | `docs/operations/incident-response.md`, `rollback.md` | Statically validated documentation | Run a failure drill and retain redacted evidence |
