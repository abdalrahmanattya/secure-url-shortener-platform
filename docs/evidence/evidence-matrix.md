...
|---|---|---|---|
| FastAPI service and Compose topology exist | `src/`, `compose.yaml`, `Dockerfile` | Statically validated | Run Compose and retain logs/health output |
...
| Local persistence schema exists | `migrations/versions/001_create_links.py` | Statically validated | Run Alembic against real PostgreSQL |
| API lifecycle behavior | `tests/test_api.py` | Locally verified: 15 passed; 2 skipped | Retain a future PostgreSQL/API run record |
...
| Workflow syntax and action pins | `scripts/ci/validate-workflows.sh`, `.github/workflows/` | Locally verified | Run the full CI workflow |
...
| Image architecture and CI build platform | `Dockerfile`, `.github/workflows/ci.yml` | Locally verified for a multi-platform base digest | Verify the hosted CI build platform during CI |
...
| Non-root container intent | `Dockerfile` | Statically validated | Build and inspect the image |
| Dependency reproducibility | `requirements.lock` | Statically validated | CI install from lock and vulnerability scan |
...
locally verified service, locally checked target structure, and explicit
evidence gates. It is not a claim of production readiness. The next proof is a
clean Linux CI run covering provider validation, image build/scans, and
PostgreSQL API evidence, followed by a separately approved bounded cloud
demonstration.
