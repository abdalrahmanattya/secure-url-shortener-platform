...
| Local persistence schema exists | `migrations/versions/001_create_links.py` | Alembic migration against real PostgreSQL verified in hosted CI run `31583275274` | Retain future migration/recovery evidence |
...
| Workflow syntax and action pins | `scripts/ci/validate-workflows.sh`, `.github/workflows/`, hosted run `31583275274` | Hosted CI verified | Preserve the retained run as CI evidence |
...
| Image architecture and CI build platform | `Dockerfile`, `.github/workflows/ci.yml`, hosted run `31583275274` | Hosted CI verified on Linux with pinned multi-platform base image | Cloud runtime architecture remains unverified |
...
| Non-root container behavior | `Dockerfile`, hosted container smoke job in `31583275274` | Hosted CI verified | Recheck after any base-image or runtime change |
| Dependency reproducibility | `requirements.lock`, hosted Python/container jobs in `31583275274` | Hosted CI verified | Continue reviewing dependency updates |
